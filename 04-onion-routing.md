...
The packet consists of four sections:
 ...
 - a 33-byte compressed `secp256k1` `public_key`, used during the shared secret
   generation
 - a 1300-byte `hop_payloads` consisting of multiple, variable length, payloads
   or up to 20 fixed sized legacy `hop_data` payloads.
...
Where, the `length`, `hop_paylod` (with contents dependent on `length`), and `HMAC` are repeated for each hop;
...
generates `2*1300` pseudo-random bytes (using the _rho_-key), and applies the result, using `XOR`, to the copy of the `hops_data`.
